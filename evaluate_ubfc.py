...
for i in range(38, 50):
    ...
    # 只要你使用了这个data的预测值，那结果就会很好，说明我们的真值是没问题的
    data = np.load(f'./results/2/5/subject{i}.npy', allow_pickle=True)
    data2 = np.load(f'./results2/1/1/subject{i}.npy', allow_pickle=True)
    data3 = np.load(f'./results2/3/2/subject{i}.npy', allow_pickle=True)
    data4 = np.load(f'./results2/3/3/subject{i}.npy', allow_pickle=True)
    data5 = np.load(f'./results2/4/2/subject{i}.npy', allow_pickle=True)
    data6 = np.load(f'./results/2/29/subject{i}.npy', allow_pickle=True)
    
    rppg_list = data2.item().get('rppg_list')
    bvp_list = data2.item().get('bvp_list')
    
    # print(np.array_equal(data.item().get('rppg_list'), data3.item().get('rppg_list')))
    # 下面这句代码野证明了我们的真值是没有变过的，唯一的区别是我们的预测值是变化了
    # print(np.array_equal(data.item().get('bvp_list'), data2.item().get('bvp_list')))
