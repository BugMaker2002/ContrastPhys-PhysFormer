...
class ViT_ST_ST_Compact3_TDC_gra_sharp(nn.Module):
    ...
    def __init__(
        ...
        self.ConvBlockLast = nn.Conv3d(dim//2, 1, 1,stride=1, padding=0)
    ...
    def forward(self, x, gra_sharp):
        ...
        # print("Trans_features, Score1: ", Trans_features.shape, Score1.shape)
        
        Trans_features2, Score2 =  self.transformer2(Trans_features, gra_sharp)  # [B, 4*4*40, 64]
        # print("Trans_features2, Score2: ", Trans_features2.shape, Score2.shape)
        
        Trans_features3, Score3 =  self.transformer3(Trans_features2, gra_sharp)  # [B, 4*4*40, 64]
        # print("Trans_features3, Score3: ", Trans_features3.shape, Score3.shape)
        ...
        # print("upsample: ", features_last.shape)
        features_last = self.upsample2(features_last)		    # x [B, 32, 7*7, 160]
        # print("upsample2: ", features_last.shape)
        
        x = self.ConvBlockLast(features_last)    # x [B, 1, 160]
        # print("rPPG: ", x.shape)
...
if __name__ == '__main__':
    model = ViT_ST_ST_Compact3_TDC_gra_sharp(S=4, dim=96, ff_dim=144, num_heads=4, num_layers=12, 
                                             image_size=(160, 128, 128), patches=(4, 4, 4), theta=0.7, dropout_rate=0.1)
    x = torch.rand(size=(2, 3, 160, 128, 128))
    print(model(x, 2.0).shape)
